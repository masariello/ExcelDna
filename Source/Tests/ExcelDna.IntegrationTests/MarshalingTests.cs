...
namespace ExcelDna.IntegrationTests
{
    public class MarshalingTests
    {
        ...
        public static string testErrorMessage([ExcelArgument(AllowReference = true)] object where)
        {
            var cellException = ExcelIntegration.CellException(where);
            return cellException == null ? "" : cellException.Message;
        }
